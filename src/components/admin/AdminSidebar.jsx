...
const AdminSidebar = ({ sidebarOpen, setSidebarOpen, isDarkMode, isRtl }) => {
  ...
  const menuItems = [
    ...
    {
      name: t("guide.applications", "Guide Applications"),
      path: "/admin/guide-applications",
      icon: FaClipboardList,
    },
    // { name: t("admin.settings"), path: "/admin/settings", icon: FaCog }, // Commented out - page not ready
  ];
  ...
  return (
    ...
    >
      {/* Logo */}
      <div
        className={`h-16 flex items-center justify-between px-4 border-b ${borderColor}`}
      >
        {sidebarOpen && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#D5B36A] font-semibold">
              {t("admin.adminPanel")}
            </span>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className={`text-xs ${secondaryText} truncate`}>
                  {user?.role}
                </p>
              </div>
            </div>
