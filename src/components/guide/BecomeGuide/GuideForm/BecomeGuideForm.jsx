  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div className={`${bgClass} rounded-xl p-6 md:p-8 shadow-lg`}>
        ...
        {existingApp && (
          <div className={`mb-6 p-4 rounded-lg ${bgClass}`}>
            <p className="text-sm text-(--text-secondary)">{t("guide.applicationStatus", `Status: ${existingApp.status}`)}</p>
          </div>
        )}
        ...
        <form onKeyDown={(e) => { if (e.key === "Enter") e.preventDefault(); }}>          
          ...
          {currentStep === 4 && (
            <StepDocuments existingCertificates={existingCertificates} certificateFiles={certificateFiles}
              documentFiles={documentFiles} handleCertificateSelect={handleCertificateSelect}
              handleDocumentSelect={handleDocumentSelect} handleRemoveCertificateFile={handleRemoveCertificateFile}
              handleRemoveDocumentFile={handleRemoveDocumentFile} handleDeleteExistingCertificate={handleDeleteExistingCertificate}
              loading={loading} t={t} labelClass={labelClass} bgClass={bgClass}
            />
          )}
