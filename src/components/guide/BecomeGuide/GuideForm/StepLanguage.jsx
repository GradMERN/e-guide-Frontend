      ...
      <div>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
          {languageOptions.map((lang) => {
            ...
            return (
              <label key={lang} className={`flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-lg border-2 cursor-pointer transition-all ${isSelected ? "border-(--primary) bg-(--primary)/10" : "border-(--border) hover:border-(--primary)/50"}`}>
                <input type="checkbox" checked={isSelected} onChange={() => toggleLanguage(lang)} className="w-4 h-4 sm:w-5 sm:h-5 rounded border-(--border) text-(--primary) shrink-0" style={{ accentColor: "var(--primary)", borderColor: "var(--border)", }}/>
