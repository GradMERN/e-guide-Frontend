  ...
  return (
      <div className="max-w-full sm:max-w-3xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto space-y-4 sm:space-y-6 md:space-y-8">
        {categories.map((category, catIndex) => {
          ...
          return (
            <motion.div key={catIndex} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.3, delay: catIndex * 0.2 }} className="rounded-2xl shadow-2xl">
              ...
              <AnimatePresence initial={false}>
                {isOpenCategory && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.1, ease: "easeInOut" }} className="overflow-hidden">
