  ...
  if (type === "guide") {
    return (
      <div className="w-full max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {[...Array(count)].map((_, i) => (
          <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="animate-pulse flex flex-col items-center">
            <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-linear-to-br from-primary/20 to-secondary/20 mb-4 shadow-xl border-4 border-surface" />
            <div className="w-full space-y-3 flex flex-col items-center">
              <div className="h-5 bg-primary/30 rounded-lg w-1/2"></div>
              <div className="h-3 bg-border rounded w-1/3"></div>
              <div className="flex gap-2 mt-2">
                <div className="h-4 w-4 bg-yellow-500/20 rounded-full"></div>
    ...
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} className="w-full max-w-2xl mx-auto text-center py-8 sm:py-12 px-4">
      <div className="relative bg-surface border-2 border-primary/30 rounded-2xl p-6 sm:p-8 shadow-xl overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-text mb-2 sm:mb-3">{title || t("common.loadError")}</h3>
          <p className="text-text-secondary text-sm sm:text-base mb-4 sm:mb-6 leading-relaxed px-2">{error}</p>
          {onRetry && (
    ...
    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-3xl mx-auto text-center py-12 sm:py-16 px-4">
      <div className="relative bg-surface border border-border rounded-2xl p-8 sm:p-12 lg:p-16 shadow-xl overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-text mb-3 sm:mb-4 px-2">{title}</h3>
          <p className="text-text-secondary text-sm sm:text-base lg:text-lg mb-6 sm:mb-8 max-w-xl mx-auto leading-relaxed px-2">{message}</p>
