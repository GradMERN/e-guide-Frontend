...
export default function LoginPage() {
    ...
    const LeftCards = [
        { icon: GiEgyptianTemple, title: "Ancient Wonders", text: "Explore the magnificent pyramids, temples, and tombs that have stood for thousands of years." },
        { icon: FaScroll, title: "Expert Guides", text: "Learn from Egyptologists who bring ancient history to life with captivating stories." },
        { icon: FaShip, title: "Nile Cruises", text: "Sail the legendary Nile River in luxury while visiting historical sites along the way." },
    ];
    ...
    return (
        <>
            ...
            <section className="flex flex-row bg-[#050505] text-slate-200 overflow-hidden">
                ...
                {/* LEFT SECTION */}
                <div className={`hidden lg:flex w-1/2 flex-col items-center justify-center space-y-20 py-40 pt-30 px-20 transition-all duration-1000 ease-out ${animate ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
                    ...
                    <div className="flex flex-col w-full max-w-lg space-y-8">
                        {LeftCards.map((item, idx) => (
                            <div key={idx} className="flex w-full gap-5 rounded-xl border border-[#f7c95f]/20 bg-[#171616]/60 p-5 transition-all duration-300 hover:-translate-y-1 hover:border-[#f7c95f] hover:bg-[#1f1e1e] hover:shadow-[0_4px_20px_rgba(247,201,95,0.1)] hover:cursor-pointer">
                    ...
                    <div className="flex space-x-12">
                        {Stats.map((stat, idx) => (
                            <div key={idx} className="text-center">
                                ...
                                <p className="text-xs uppercase tracking-wider text-[#bfb191]">{stat.label}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                {/* RIGHT SECTION */}
                <div className={`w-full lg:w-1/2 flex items-center justify-center bg-[#130f0c] py-40 pt-50 px-3 lg:px-10 transition-all duration-1000 ease-out delay-100 ${animate ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}>
                            ...
                            {({ isSubmitting, isValid, dirty }) => (
                                <Form className="flex flex-col gap-4 sm:gap-5 md:gap-6">
                                    ...
                                    <div className="flex justify-end -mt-3 sm:-mt-4.5 md:-mt-5">
                                        <Link to="#" className="text-xs text-[#bfb191] transition-colors hover:text-[#f7c95f]">
                                            Forgot Password?
                                        </Link>
                                    </div>
                                    ...
                                    >
                                        {isSubmitting ? <GiEgyptianProfile className="h-5 w-5 animate-spin text-black" /> : <> <FaSignInAlt /> <span>Sign In</span> </>}
                                    </button>
                                    ...
                                    <button onClick={()=> window.location.href = "http://localhost:3000/api/auth/google"} type="button" className="group flex w-full items-center justify-center gap-3 rounded-xl border border-[#2b2b2b] bg-[#1a1a1a] py-3 text-white cursor-pointer transition-all duration-300 hover:bg-[#252525] hover:border-[#f7c95f]/50">
