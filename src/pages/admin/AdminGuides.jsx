  ...
  const fetchGuides = async () => {
    try {
      ...
      for (const guide of guidesList) {
        try {
          const toursResponse = await axiosClient.get(
            `/tours/guide/${guide._id}`
          );
          setGuideTours((prev) => ({
            ...prev,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`${cardBg} border ${borderColor} rounded-lg p-4`}>
          <p className={`${secondaryText} text-sm mb-1`}>
            {t("admin.guides.totalTours")}
          </p>
          <p className="text-2xl font-bold text-amber-500">
            {Object.values(guideTours).reduce(
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {(guideTours[guide._id] || []).slice(0, 3).map((tour) => (
                      <p
                        key={tour._id}
                        className={`text-xs ${secondaryText} truncate`}
                      >
                        • {tour.name} ({tour.city})
                      </p>
                    ))}
